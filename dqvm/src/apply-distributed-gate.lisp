...
(defun apply-distributed-gate (qvm instr &key (blocks-per-chunk *default-blocks-per-chunk*))
  (with-slots (addresses amplitudes scratch)
    ...
    (with-slots (number-of-processes number-of-qubits permutation)
        addresses
...
(defun make-arrays (addresses)
  "Create N empty arrays to store offsets."
  (with-slots (number-of-processes blocks-per-process remainder-blocks block-size)
      addresses

    (let ((dimension (* (+ blocks-per-process
                           (boolean-bit remainder-blocks))
                        block-size)     ; Conservative upper bound.
                     ))
...
(defun non-blocking-receive (qvm next-permutation start-offset end-offset requests)
  ...
  (let* ((addresses (addresses qvm))
         (all-recv-offsets (make-arrays addresses)))
...
(defun non-blocking-send (qvm next-permutation start-offset end-offset requests)
  ...
  (loop :with addresses := (addresses qvm)
        :with all-send-offsets := (make-arrays addresses)
